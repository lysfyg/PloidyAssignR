...
PloidyAssignR_run_all(
  input_path_data = "path/to/strandseq/count",
...
This plot can be used to visually evaluate the ploidy assignment results and distinguish regions with more homogenous or heterogenous distribution patterns. 
...
```{r Ploidy Summary, message = FALSE, echo = FALSE}
...
1. homogenous strand state distribution pattern/clustering
...
### Single Cell Heatmap
To visualize the single cell copy number within the context of the cell population we use a heatmap that uses hierachical clustering to group cells. See an example plot of Chr2 for K562 below.
...
1. Heterogenous distribution pattern 
